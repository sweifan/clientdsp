...
class DbUtil {
    ...
    const TAB_ACCOUNT               = 'dsp_accountinfo';
    const TAB_PROINFO               = 'dsp_proinfo';
    ...
    public function __call($strFuncName, $arrParams = []) {
        ...
        if (empty($arrParams)) {
            return $this->{$arrAcT[1]}(self::TAB_MAP[strtolower($arrAcT[2])], []);
        } else {
    ...
    private function udp($strTabName, $arrParams) {
        ...
        if ($this->CI->db->affected_rows() === 0) {
            throw new Exception('nothing changed', ErrCode::OK);
        }
        return true;;
    }
